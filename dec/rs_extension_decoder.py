...
class RSExtensionFieldDecoder:
    ...
    msg_size=0
    def __init__(self,field_size,locators,multipliers,receivedCodeWord,msgSize,degree,polynom):
        #self.=Codeword(field_size,locators,multipliers,received_codeword=receivedCodeWord)
        self.field_size = field_size
        self.msg_size=msgSize
        self.codeword_length=len(receivedCodeWord)
        ...
        self.field_size = field_size
        self.received_codeword = receivedCodeWord
        self.mod_field_size=extension_field(degree,field_size,polynom)
    
    def get_interpolate_points(self,locators,receivedCodeWord):
        ...
        for i in locators:
            tempArray=[]
            tempArray.append(i)
            tempArray.append((receivedCodeWord[count]))
    ...
    def __calculate_polynoms(self,degQ0,degQ1,locators):
        ...
        for i in range(0,n):
            ...
            for j in range(0,degQ0+1):
                tempArray.append(self.mod_field_size((locators[i]**j)))
            rows1.append(tempArray)
        
        for i in range(0,n):
            tempArray=[]
            for j in range(0,degQ1+1):
                tempArray.append(self.mod_field_size((self.received_codeword[i]*(locators[i]**j))))
    ...
    def __add_minus_one(self,length,element_index):
        ...
        for i in range(0,length):
            array_to_return.append(self.mod_field_size(0))
            if i==element_index:
                array_to_return[i]=self.mod_field_size(-1)
    ...
    def __calculate_coefficients(self, res_row):
        ...
        pprint(Matrix(res_row))
        print(res_row)
        all_zero_row = [self.mod_field_size(0)]*len(res_row[0])
        rm_indexes = []
        count = len(res_row)-1
        c1=0
        while count>=0:
            if res_row[count] == all_zero_row:
                print("Hier",res_row[count])
                c1+=1
                if c1!=1 :
                    res_row.remove(all_zero_row)
                else:
                    count = count-1
                    continue
        ...
        if len(res_row)!=len(res_row[0]):
            ...
            for i in range(0,count):
                ...
                if i!=len(res_row[0])-1:
                    if len(res_row)-1>=i:
                        if res_row[i][i]!=self.mod_field_size(1):
                        ...
                        if(len(rm_indexes)!=0  and res_row[i-1][i-1]==self.mod_field_size(-1)):
                                print("flag")
                                rm_indexes.remove(i)
                        colT=self.__add_minus_one(len(res_row[0]),i)
                        res_row.insert(i,colT)
        ...
        print("Polynom elems:",polynomElems)
        i = 0
        return polynomElems
    ...
    def __to_reduced_row_echelon_form(self, M):
        ...
        for r in range(rowCount):
            if lead >= columnCount:
                return
            i = r
            while M[i][lead] == self.mod_field_size(0):
                i += 1
                if i == rowCount:
                    i = r
                    lead += 1
    ...
    def __divide_polynoms(self,polynomElems):
        ...
        q0Elements=[]
        q1Elements=[]
        if self.codeword_length != (len(polynomElems)-2):
            count = len(polynomElems) -self.codeword_length- 2 
            print(count)
        print("Q Elements: ", polynomElems)
        for i in range(0,self.deg_Q0+1):
            q0Elements.append(polynomElems[i]*self.mod_field_size(-1))
        ...
        q0Elements.reverse()
        q1Elements.reverse()
        print("Q0 Elements ", q0Elements)
        print("Q1 Elements ", q1Elements)
        #x = np.array(q0Elements)
        #y = np.array(q1Elements)
            ...
            if coef != 0: # useless to multiply if coef is 0
                for j in range(1, len(divisor)): # in synthetic division, we always skip the first coefficient of the divisor,
                    ...
                    if isinstance(divisor[j], int): 
                        print("coef",type(coef),"divisor[",j,"]",type(divisor[j]))
                        out[i + j] += self.mod_field_size(-1)*self.mod_field_size(divisor[j]) * coef
                    else: out[i + j] += self.mod_field_size(-1)*(divisor[j]) * coef
    ...
    def __decode_codeword(self):
        ...
        resultedRows=self.__calculate_polynoms(self.deg_Q0,self.deg_Q1,self.locators)
        polynomElems=self.__calculate_coefficients(resultedRows)
        polynomCoefficients=self.__divide_polynoms(polynomElems)
        self.message=polynomCoefficients
        print(self.message)
        return self.message

    def decode(self):
        return self.__decode_codeword()
